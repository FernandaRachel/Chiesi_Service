        ...
        public override void Calculate(Text txt)
        {
            ...
            if ((index) < operationInfos.Count())
            {
                ...
                x = CreateString(String.Format(changeDotToComma, "{0:0.0}", gli.GliQty), String.Format(changeDotToComma, "{0:0.0}", flux.TheoricQty), String.Format(changeDotToComma, "{0:0.0}", flux.RealQty / 100), flowvariation,
                    String.Format(changeDotToComma, "{0:0.0}", flux.Limit), String.Format(changeDotToComma, "{0:0.0}", gli.GliQty / 100), cellVariation, cell.Limit.ToString(), gli.OutFlowStart.ToString("HH:mm:ss"), gli.OutFlowEnd.ToString("HH:mm:ss"));
            }
            else
            {
                gerarPdf = true;
